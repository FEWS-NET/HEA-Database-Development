...
-- HEA requires a user and schema for use by Dagster
\set APP dgs
\set SCHEMA :APP '_owner'
\connect :DATABASE
CREATE SCHEMA :SCHEMA;
...
CREATE ROLE :DAGSTER PASSWORD :'POSTGRES_PASSWORD' LOGIN CREATEDB NOCREATEROLE NOSUPERUSER;
GRANT :DAGSTER TO :OWNER;
GRANT CONNECT ON DATABASE :DATABASE TO :DAGSTER;
GRANT CREATE ON DATABASE :DATABASE TO :DAGSTER;
ALTER SCHEMA :SCHEMA OWNER TO :DAGSTER;
ALTER USER :DAGSTER SET search_path=:SCHEMA,public,pg_temp;
\connect postgres
