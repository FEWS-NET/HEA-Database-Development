...
if env("LOG_FORMATTER") == "json":
    access_log_format = (
        '{ "time_local":"%(t)s", "remote_addr":"%(h)s", "request":"%(U)s", "query":"%(q)s", "request_method":"%(m)s", '
        '"status":%(s)s, "body_bytes_sent":%(B)d, "request_time":%(D)d, "http_user_agent":"%(a)s", '
        '"http_referrer":"%(f)s", "x_forwarded_for": "%({X-Forwarded-For}i)s"}'
    )
else:
