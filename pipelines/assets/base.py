...
def corrected_files(context: AssetExecutionContext, config: BSSMetadataConfig) -> Output[BytesIO]:
    ...
    def validate_previous_value(worksheet_name, cell_reference, expected_previous_value, previous_value):
        ...
        if isinstance(previous_value, (datetime.date, datetime.datetime)) or isinstance(
            expected_previous_value, (datetime.date, datetime.datetime)
        ):
            ...
            previous_value = (
                ...
                if isinstance(previous_value, (datetime.date, datetime.datetime))
                else str(previous_value).strip()
            )
            expected_previous_value = (
                expected_previous_value.isoformat()
                if isinstance(expected_previous_value, (datetime.date, datetime.datetime))
