...
def update_common_name_from_fdw(ClassifiedProduct):
    """
    Update the records based on an extract of current data from FDW to add the common name
    """
    logging.info("Updating common name from fdw ...")
    records_updated = 0
    records_skipped = 0
    response = requests.get("https://fdw.fews.net/api/classifiedproduct/?format=json")
    fdw_products = response.json()
    for classified_product in ClassifiedProduct.objects.all():
        ...
        if fdw_product and len(fdw_product) == 1:
            ...
            records_skipped += 1
            logging.info(f"Skipping ... failed to fetch {classified_product.cpcv2} from FDW")
...
def update_kcals_per_unit(ClassifiedProduct, UnitOfMeasure):
    ...
    kcal_file = os.path.join(os.path.dirname(__file__), "kilocalorie_table.txt")
    records_updated = 0
    records_failed = 0
    records_skipped = 0
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("common", "0004_alter_classifiedproduct_unit_of_measure"),
    ]
