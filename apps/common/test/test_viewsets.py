...
class CountryViewSetTestCase(APITestCase):
    ...
    def setUpTestData(cls):
        cls.existing_county_count = Country.objects.count()
    ...
    def test_list_returns_all_records(self):
        ...
        self.assertEqual(len(response.data), 4 + self.existing_county_count)

    def test_filter_by_single_country_code(self):
        response = self.client.get(self.url, {"country_code": self.country1.iso3166a2})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(self.country1.iso3166a2, json.loads(response.content)[0]["iso3166a2"])
...
class CurrencyViewSetTestCase(APITestCase):
    @classmethod
    def setUpTestData(cls):
        cls.existing_currency_count = Currency.objects.count()
    ...
    def test_list_returns_all_records(self):
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(json.loads(response.content.decode("utf-8"))), self.existing_currency_count + 3)
...
class UnitOfMeasureViewSetTestCase(APITestCase):
    @classmethod
    def setUpTestData(cls):
        cls.all_units = UnitOfMeasure.objects.count()
        cls.volume_units = UnitOfMeasure.objects.filter(unit_type=UnitOfMeasure.VOLUME).count()
    ...
    def test_list_returns_all_records(self):
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(json.loads(response.content.decode("utf-8"))), self.all_units + 6)

    def test_list_returns_filtered_data_by_unit_type(self):
        response = self.client.get(self.url, {"unit_type": UnitOfMeasure.VOLUME})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(json.loads(response.content.decode("utf-8"))), self.volume_units + 3)
